...
if ( count( $versions ) <= 0 ) {
	...
	<section class="download-information">
		<p><?php _e( 'No files found for this download.', 'dlm_page_addon' ); ?></p>
	</section>
	<?php
	return;
}
...
<section class="download-information">
	<aside>
		...
		<?php
		$download_button = apply_filters( 'dlm_page_addon_download_button', '<a class="aligncenter download-button" href="' . $dlm_download->get_the_download_link() . '" rel="nofollow">' . __( 'Download', 'dlm_page_addon' ) . '</a>', $dlm_download->get_id() );
		if ( '' !== $download_button ) {
	...
	<article>
			...
			<?php
                ...
                if ( sizeof( $versions ) > 1 ) {
                    ...
                    foreach ( $versions as $version ) {
                        ...
                        $previous_versions .= '<li><a href="' . $dlm_download->get_the_download_link() . '">' . sprintf( __( 'Version %s', 'dlm_page_addon' ), $dlm_download->get_version()->get_version_number() ) . '</a> - ' . date_i18n( get_option( 'date_format' ), strtotime( $version_post->post_date ) ) . '</li>';
                    }
		...
		<?php echo wpautop( wptexturize( do_shortcode( $dlm_download->get_description() ) ) ); ?>
