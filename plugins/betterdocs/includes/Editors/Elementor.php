...
class Elementor extends BaseEditor {
	...
	public function betterdocs_query( $wb, $taxonomy ) {
		...
		} elseif ( $wb->get_name() !== 'betterdocs-sidebar' ) {
			$wb->add_control(
				'box_per_page',
				[
					'label'   => $wb->get_name() == 'betterdocs-tab-view-list' ? __( 'Tabs Per Page', 'betterdocs' ) : __( 'Box Per Page', 'betterdocs' ),
					'type'    => Controls_Manager::NUMBER,
					'default' => '8'
				]
			);
	...
	public function builder_widget_list() {
		return apply_filters(
			'betterdocs_elementor_pro_widgets',
			[
	...
	public function modified_ajax_action( $ajax ) {
		if ( ! isset( $_REQUEST['actions'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Missing
			return;
		}

		$actions = json_decode( stripslashes( $_REQUEST['actions'] ), true ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
		$data    = false;
