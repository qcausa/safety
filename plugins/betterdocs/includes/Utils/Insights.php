...
class Insights {
	...
	private function redirect_to() {
        $request_payload = isset( $_SERVER['REQUEST_URI'] ) ? $_SERVER['REQUEST_URI'] : ''; //phpcs:ignore -- Slash Is Needed Here Because This Is An Url
	...
	public function clicked( $notice = null ) {
		if ( isset( $_GET['_wpnonce'] ) && isset( $_GET['plugin'] ) && isset( $_GET['plugin_action'] ) ) {
            ...
            if( ! wp_verify_nonce( $_GET[ '_wpnonce' ], '_wpnonce_optin_' . $this->plugin_name ) ) { //phpcs:ignore -- Nonce Sanitization Is Not Needed Here, Because This Is Generated By WP Core Function
				return;
			}

            $plugin = sanitize_text_field( $_GET['plugin'] ); //phpcs:ignore -- Text's Are Sanitized Here, Not Strings With Url's
            $action = sanitize_text_field( $_GET['plugin_action'] ); //phpcs:ignore -- Text's Are Sanitized Here, Not Strings With Url's
			if ( $action == 'yes' ) {
				$this->schedule_tracking();
	...
	public function deactivate_reasons_form_submit() {
		...
		if ( isset( $_POST['values'] ) ) {
            $values = sanitize_text_field( $_POST['values'] );  //phpcs:ignore -- Text's Are Sanitized Here, Not Strings With Url's
			update_option( 'wpins_deactivation_reason_' . $this->plugin_name, $values );
		}
		if ( isset( $_POST['details'] ) ) {
            $details = sanitize_text_field( $_POST['details'] );  //phpcs:ignore -- Text's Are Sanitized Here, Not Strings With Url's
