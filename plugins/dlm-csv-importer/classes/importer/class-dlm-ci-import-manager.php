...
class DLM_CI_Import_Manager {
	 ...
	 * @return bool
	 */
	private function attach_thumbnail_to_download( $thumbnail_id, $download_id ) {

		if ( !empty( $thumbnail_id ) && absint( $thumbnail_id ) > 0 ) {

			return add_post_meta( $download_id, '_thumbnail_id', $thumbnail_id, true );
		}
		...
		if ( count( $downloads ) > 0 ) {
			...
			foreach ( $downloads as $import_download ) {
				...
				if ( count( $import_download->get_meta() ) > 0 ) {
					foreach ( $import_download->get_meta() as $meta_key => $meta_values ) {
				...
				// Thumbnail
				$this->attach_thumbnail_to_download( $import_download->get_thumbnail(), $new_download->get_id() );
				$post_args = array(
					'ID'        => $new_download->get_id(),
					'post_date' => $import_download->get_published_date(),
				);
				// Update Download slug also, in case it has been changed
				if ( ! empty( $import_download->get_slug() ) ) {
					$post_args['post_name'] = sanitize_title( $import_download->get_slug() );
				}
				...
				if ( count( $import_download->get_versions() ) > 0 ) {
					...
					if ( count( $current_versions ) > 0 ) {
						...
						// loop
						/** @var DLM_Download_Version $current_version */
						foreach ( $current_versions as $current_version ) {

							// delete current version
							wp_delete_post( $current_version->get_id(), true );
						}
					}
					...
					foreach ( $import_download->get_versions() as $version ) {
						...
						// Set the meta download count because the table that handles the download count is not imported.
						if ( method_exists( $new_version, 'set_meta_download_count' ) ) {
							$new_version->set_meta_download_count( absint( $version->get_download_count() ) );
						} else {
