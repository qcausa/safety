...
class PMXI_Import_Record extends PMXI_Model_Record {
                    ...
                    if ( $is_images_to_delete ) {
                        switch ($this->options['custom_type']) {
                            ...
                            case 'taxonomies':
                                ...
                                break;
                            default:
                                $this->importer->delete_images($articleData['ID'], $articleData, $images_bundle);
                                break;
                        }
                    }
				}
							...
							foreach ( $image_custom_fields as $image_custom_field ) {
								switch ( $this->options['custom_type'] ) {
									...
									case 'gf_entries':
										...
										break;
									default:
                                        $this->importer->delete_image_custom_field($pid, $image_custom_field);
										break;
								}
							}
						}
                        ...
                        foreach ($images_sources as $source_type => $images){
                            ...
                            switch ($this->options['custom_type']){
                                ...
                                case 'taxonomies':
                                    ...
                                    break;
                                default:
                                    $this->importer->update_content($pid, $articleData['post_content']);
                                    break;
                            }
                        }
                    }
    ...
    public function downloadFile($url, $image_filepath, $is_cron, $logger, $type = 'images'){
	    ...
	    if ( (is_wp_error($request) || !array($request)) and ! @file_put_contents($image_filepath, @file_get_contents($url, false, $get_ctx))) {
		    ...
		    !empty($request['body']) && @file_put_contents($image_filepath, $request['body']);
