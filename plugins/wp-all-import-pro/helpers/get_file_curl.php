...
if ( ! function_exists( 'get_file_curl' ) ):
	...
	function get_file_curl( $url, $fullpath, $to_variable = false, $iteration = false ) {
		...
		// wp_safe_remote_get doesn't actually block meta URLs for services like DigitalOcean and AWS.
		$response = wp_safe_remote_get( $url, array( 'redirection' => 5 ) );

		// Validate redirected URLs to ensure we do block access to those meta URLs.
		if ( ! is_wp_error( $response ) ) {
			$http_response = $response['http_response'];
			if ( method_exists( $http_response, 'get_response_object' ) ) {
				$response_object = $http_response->get_response_object();
				if ( isset( $response_object->history ) && is_array( $response_object->history ) ) {
					foreach ( $response_object->history as $redirect_response ) {
						if ( isset( $redirect_response->headers['location'] ) ) {
							$redirect_url = $redirect_response->headers['location'];
							if ( pmxi_is_private_ip( $redirect_url ) ) {
								return false;
							}
						}
					}
