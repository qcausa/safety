...
class Query {
	...
	public function __construct( $query ) {
		...
		$results_post__in = $this->get_updated_post__in( $query_args, $post__in );
	...
	public function search( $field ) {
		...
		$results = $wpdb->get_results( $wpdb->prepare( 'SELECT object_id, parent_id FROM %i WHERE field_id = %d AND value LIKE %s', $table_name, $field_id, '%' . $field_value . '%' ) );
