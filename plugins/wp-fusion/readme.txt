...
* Improved - Added a notice to the WPForo WP Fusion settings page in the admin to indicate when the forum base page is protected by WP Fusion
...
* Fixed a fatal error during MemberPress registration on PHP 8.1 when "Remove tags (above) when cancelled" was selected on a membership product, but no tags were specified for the membership product
...
* Improved - If a WooCommerce subscription is canceled, put on hold, or expired, and Remove Tags is checked, WP Fusion will first check the customer's other active subscription products to see if they apply the same tags. Any tags applied by products where the customer still has an active subscription [will not be removed](https://wpfusion.com/documentation/ecommerce/woocommerce-subscriptions/#concurrent-subscriptions).
* Fixed Filter Course Steps not working properly on LearnDash topics
* Fixed BuddyPress / BuddyBoss XProfile fields updated by Fluent Forms not syncing with the CRM
* Fixed import tool not working with Dynamics 365
* Fixed mapped "Meta" fields on Gravity Forms feeds getting synced to the CRM even when empty
* Fixed undefined constant `WPB_PLUGIN_DIR` with older versions of WPBakery
