...
class Page {
	...
	/**
	 * Keeps a list of multi valued field.
	 */
	private $multivalued_fields = array();
	 ...
	 */
	public function __construct( $page, $title = '', $defaults = array() ) {

		$this->page             = $page;
		$this->title            = $title;
		$this->default_settings = $defaults;
	...
	public function init() {
		...
		if ( ! $this->using_unique_option() ) {
			...
			if ( false === get_option( $global_option_name ) ) {
				add_option( $global_option_name, $this->default_settings );
			}
		}
		...
		foreach ( $this->panels as $panel ) {
			...
			foreach ( $sections as $section ) {
				...
				foreach ( $section->get_fields() as $field ) {
					...
					if ( $field->is_multi_valued_field() ) {
						$this->multivalued_fields[ $field->get_id() ] = $field;
					}
					...
					if ( $this->using_unique_option() ) {
						...
						if ( false === get_option( $field->get_name() ) ) {
							add_option( $field->get_name(), $field->get_default() );
						}
			...
			if ( ! $this->using_unique_option() ) {
				...
				// filter settings value to limit the scope of update.
				add_filter(
					'pre_update_option_' . $this->get_option_name(),
					array( $this, 'limit_settings_update_scope' ),
					10,
					3
				);
			}
	...
	/**
	 * Filters settings option to limit the scope of update.
	 *
	 ...
	 * @return array
	 */
	public function limit_settings_update_scope( $value, $old_value, $option ) {

		$fkey = sanitize_key( 'pt-settings-fields-' . $this->get_option_name() );

		if ( ! is_array( $value ) || empty( $_POST[ $fkey ] ) ) {
			return $value;
		}

		$fields = wp_unslash( $_POST[ $fkey ] );
		...
		foreach ( $fields as $field ) {
			if ( isset( $value[ $field ] ) ) {
				...
				// this is a multi valued field, store an empty array to remember that user did update settings.
				if ( isset( $this->multivalued_fields[ $field ] ) ) {
					$old_value[ $field ] = array();
				} else {
					unset( $old_value[ $field ] );
				}
			}
		}
		...
		<div class="metabox-holder pt-settings-metabox-holder">
			<div class="postbox options-postbox pt-options-box">
				<form method="post" action="<?php echo esc_url( admin_url( 'options.php' ) ); ?>">
					...
					<?php foreach ( $this->panels as $panel ) : ?>
						...
						<div id="<?php echo esc_attr( $panel->get_id() ); ?>" class="pt-settings-panel-tab">
							...
							<?php foreach ( $sections as $section ) : ?>
								...
								<div id="<?php echo esc_attr( $section_id ); ?>" class="pt-settings-section-block <?php echo esc_attr( $section_id ); ?>">
					...
					<?php endforeach; ?>
					<?php $fkey =  sanitize_key( 'pt-settings-fields-' . $this->get_option_name() ); ?>
					<input type="hidden" name="<?php echo $fkey; ?>" value="<?php echo $this->get_rendered_field_ids();?>" />
				</form>
				<?php $this->styles(); ?>
			</div>
		</div>
	...
	/**
	 * Get rendered field ids as comma separated list.
	 *
	 * @return string
	 */
	private function get_rendered_field_ids() {
	 ...
	 * @return Panel
	 */
	public function add_panel( $panel_id, $title, $desc = '' ) {
		$this->panels[ $panel_id ] = new Panel( $panel_id, $title, $desc );
	...
	public function get_option( $field ) {
		...
		if ( ! $this->using_unique_option() ) {
			...
			$options = $function_name( $this->get_option_name(), $this->default_settings );
