...
class Ajax_Handler {
	...
	public function create_page() {
		...
		if ( ! $key || ! current_user_can( 'manage_options' ) ) {
			wp_send_json_error(
				array( 'message' => __( 'Invalid action.', 'pt-settings' ) )
			);
		}
		...
		if ( empty( $page_details ) || ! is_array( $page_details ) || empty( $page_details['title'] ) ) {
			wp_send_json_error( array( 'message' => __( 'Page details not found.', 'pt-settings' ) ) );
		}
		...
		wp_send_json_success(
			array(
				...
				'message'    => __( 'Page created successfully.', 'pt-settings' ),
