...
//	    Environment::Boot();
	...
	public function addMenus($networkMode, $networkAdminMenu) {
		...
		if (media_cloud_licensing()->is_plan('pro')) {
			add_submenu_page('media-cloud', 'Submit Issue', 'Submit Issue', 'manage_options', 'https://docs.mediacloud.press/support/');
		}
		    ...
		    add_action('admin_footer', function(){
                ...
                <script>
                    jQuery(document).ready(function() {
                        <?php do_action('mediacloud/ui/media-detail-buttons-extra'); ?>
                        ...
                        <?php foreach($toRemove as $remove) { ?>
                        attachTemplateButtonText = attachTemplateButtonText.replace(<?php echo $remove ?>, '');
                        <?php } ?>
                        attachTemplateText = attachTemplateText.replace(attachTemplateInteriorMatch[1], "<?php echo str_replace('__ID__', '{{ data.id }}', $mediaLinksHTML) ?>"+attachTemplateButtonText);
                        ...
                        attachTemplateButtonText = attachTemplateButtonText.replace(<?php echo $remove ?>, '');
	                    <?php } ?>
                        attachTemplateText = attachTemplateText.replace(attachTemplateInteriorMatch[1], "<?php echo str_replace('__ID__', '{{ data.id }}', $mediaButtonsHTML) ?>"+attachTemplateButtonText);
                        ...
                        if (attachTemplate)
                        {
                            attachTemplate.text(attachTemplate.text().replace(/(<a class="(?:.*)edit-attachment(?:.*)"[^>]+[^<]+<\/a>)/, "<?php echo $mediaDetailLinksHTML ?>"));
                        }
                    });
                </script>
			    <?php
