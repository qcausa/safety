...
namespace MediaCloud\Plugin\Tools\Integrations\PlugIns\WPForms\Tasks;
...
use MediaCloud\Plugin\Tools\Integrations\PlugIns\WPForms\WPFormsIntegration;
use MediaCloud\Plugin\Tools\Integrations\PlugIns\WPForms\WPFormsSettings;
...
class WPFormsMigrateTask extends Task {
	...
	public static function title() {
		return 'Migrate WPForms Uploads';
	}
	...
	public static function instructionView() {
		return 'tasks.batch.instructions.wp-forms-migrate';
	}
	...
	public static function warnOption() {
		return 'migrate-wp-forms-task-warning-seen';
	}
	...
	public function prepare($options = [], $selectedItems = []) {
		...
		$submissions = $wpdb->get_results("SELECT entry_id FROM {$wpdb->prefix}wpforms_entries", ARRAY_A);
		$index = 1;
		foreach($submissions as $submission) {
			$this->addItem([
				'index' => $index,
				'id' => $submission['entry_id']
			]);
	...
	public function performTask($item) {
		...
		global $wpdb;
		$entryRow = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}wpforms_entries WHERE entry_id = %d", $entryId), ARRAY_A);
		if (empty($entryRow)) {
			return true;
		}

		$formRow = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->posts} WHERE id = %d", $entryRow['form_id']), ARRAY_A);
		if (empty($formRow)) {
			return true;
		}

		$fields = json_decode($entryRow['fields'], true);
		$form = json_decode($formRow['post_content'], true);

		$fields = WPFormsIntegration::processFormSubmission(WPFormsSettings::instance(), $fields, $form, false);
		$wpdb->update($wpdb->prefix.'wpforms_entries', ['fields' => json_encode($fields)], ['entry_id' => $entryId]);
		...
		foreach($fields as $field) {
			...
			$wpdb->update($wpdb->prefix.'wpforms_entry_fields', ['value' => $field['value']], ['entry_id' => $entryId, 'field_id' => $field['id']]);
		}
