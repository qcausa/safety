...
class StorageCommands extends Command {
	...
	public function migrate($args, $assoc_args) {
		...
		if ($media_cloud_licensing->is__premium_only()) {
			...
			if (isset($assoc_args['ignore-errors']) ) {
				unset($assoc_args['ignore-errors']);
				$assoc_args['ignore-errors'] = true;
			}
	...
	public function makewebp($args, $assoc_args) {
		if (media_cloud_licensing()->is_plan__premium_only('basic')) {
			if (!class_exists('EIO_Base')) {
				self::Error("The EWWW Image Optimizer plugin is either not installed or it's not activated.  Please install and activate it before running this command.");
				exit(0);
			}
	...
	public static function Info($string, $newline = false) {
		parent::Info($string, $newline); // TODO: Change the autogenerated stub
	}

	public static function Error($string) {
		parent::Error($string); // TODO: Change the autogenerated stub
	}
}
