		...
		if($this->enabled()) {
			...
			if (media_cloud_licensing()->is_plan__premium_only('basic')) {
				...
				if (class_exists('EIO_Base')) {
					TaskManager::registerTask(GenerateEwwwWebPTask::class);
				}
			}
	...
	public function getAttachmentURLFromMeta($meta) {
	    ...
	    if ($doSign) {
            ...
            try {
                ...
                Logger::error("Error trying to generate url for {$key}.  Message:".$ex->getMessage(), [], __METHOD__, __LINE__);
                return null;
            }
        }
	}
            ...
            if (!empty($extraFiles)) {
                ...
                foreach($extraFiles as $extraFile) {
	                ...
	                if (!$this->client->exists($extraKey)) {
		                Logger::info("\tUploading $extraKey version to S3", [], __METHOD__, __LINE__);
		                $this->client->upload($extraKey, $extraPath, $privacy, StorageToolSettings::cacheControl(), StorageToolSettings::expires(), 'image/webp');
...
MIGRATED;
	 ...
	 * @throws StorageException
	 */
	private function getLeopardURL(string $provider, $region, $bucket, $key) {
    ...
    private function importLeopardMetadata($postId, $meta, $leopardMetadata) {
	    ...
	    $s3Info = [
		    'url' => $this->getLeopardURL($provider, $region, $bucket, $key),
		    'provider' => $provider,
		    'bucket' => $bucket,
	    ...
	    if (isset($meta['sizes'])) {
		    ...
		    foreach($meta['sizes'] as $size => $sizeData) {
			    ...
			    $sizeS3Info = $s3Info;
			    $sizeS3Info['url'] = $this->getLeopardURL($provider, $region, $bucket, $sizeKey);
	    ...
	    if ($hasMeta) {
		    ...
		    update_post_meta($postId, 'ilab_s3_info', ['s3' => $s3Info]);
	    }
