...
$this->add_render_attribute( 'main-container', 'id', 'laevent_' . esc_attr($this->get_id()) );
...
<div <?php $this->print_render_attribute_string( 'main-container' ); ?> <?php $this->render_variable($masonry_attr); ?>><?php
	...
	if($the_query->have_posts()){
		...
		if( $enable_masonry ){
			$this->render_masonry_filters('#laevent_'.esc_attr($this->get_id()).' .lakit-posts__list');
		}
		...
		if( $this->get_settings_for_display('paginate') == 'yes' ){
			...
			?>
            <nav class="<?php echo esc_attr(join(' ', $nav_classes)) ?>" data-parent-container="#laevent_<?php echo esc_attr($this->get_id()) ?>" data-container="#laevent_<?php echo esc_attr($this->get_id()) ?> .lakit-posts__list" data-item-selector=".lakit-posts__item" data-ajax_request_id="<?php echo esc_attr($paged_key) ?>">
				...
				<?php
				echo paginate_links( apply_filters( 'lastudio-kit/posts/pagination_args', $pagination_args, 'la_event' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				?>
            </nav>
			<?php
		}
