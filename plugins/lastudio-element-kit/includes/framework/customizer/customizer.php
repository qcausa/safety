	...
	class CX_Customizer {
		...
		public function auto_enqueue_frontend_css(){
			...
			foreach ( (array) $this->options as $id => $option ) {
				...
				if ( 'control' === $option['type'] && !empty($option['css']) ) {
					...
					if(!empty($value)){
                        if($option['field'] == 'number' && !empty($option['unit'])){
			...
			if(!empty($css_render)){
				echo sprintf('<style id="lakitcustomizer-css-style">%1$s</style>', $css_render); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
		...
		public function init_fonts_manager() {
			...
			$this->fonts_manager->set_args( array(
				...
				'is_google_font' => array( $this, 'is_google_font' ),
		...
		public function add_control( $id, $args ) {
			...
			switch ( $field_type ) {
				...
				case 'fonts':
						...
						$control_args = wp_parse_args( array(
							'type'    => 'select',
							'choices' => ['' => __('— Select —', 'lastudio-kit')] + $choices,
						), $control_args );
					break;
		...
		public function is_google_font( $font_family ){
            ...
            foreach ( $fonts_data as $type => $file ) {
                if( $type == 'google' ){
                    ...
                    $_cache_key = sprintf('lakit_font_type_%1$s_v_%2$s', $type, lastudio_kit()->get_version(true));
                    $cache_fonts = get_transient( $_cache_key );
                    if(empty($cache_fonts)){
                        $fonts = $this->read_font_file( $file );
                        if ( is_array( $fonts ) ) {
                            $cache_fonts = $this->satizite_font_family( $fonts );
                            set_transient( $_cache_key, $cache_fonts, WEEK_IN_SECONDS );
                        }
                    }

                    if(!empty($cache_fonts) && in_array($font_family, $cache_fonts)){
                        $check = true;
                        break;
                    }
                }
		...
		public function prepare_fonts() {
			...
			foreach ( $fonts_data as $type => $file ) {
                ...
                if(!empty($cache_fonts)){
                    $this->fonts = array_merge( $this->fonts, $cache_fonts );
                }
			}
        ...
        public function custom_devices( $devices ) {
            $laptop           = array(
                'laptop' => array(
                    'label' => __( 'Enter laptop preview mode', 'lastudio-kit' ),
                ),
            );
            $mobile_landscape = array(
                'mobile_extra' => array(
                    'label' => __( 'Enter mobile landscape preview mode', 'lastudio-kit' ),
                ),
            );
